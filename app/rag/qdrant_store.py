...
class QdrantStore:
    ...
    def get_content_hashes(self, ids: list[int]) -> dict[int, str]:
        """주어진 point ID 목록의 content_hash 반환. 임베딩 캐싱에 사용."""
        if not ids:
            return {}
        try:
            ...
            return {
                r.id: r.payload.get("content_hash", "")
                for r in results
                if r.payload
            }
        except Exception as e:
            logger.warning("content_hash 조회 실패 (전체 재임베딩): %s", e)
            return {}
