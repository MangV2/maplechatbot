...
개선 사항:
- 슬라이딩 윈도우 청킹: 긴 게시글을 CHUNK_SIZE 단위로 분할하여 임베딩
- 임베딩 캐싱: content_hash 비교로 변경 없는 청크 재임베딩 스킵
- 해시 충돌 최소화: SHA256 전체 8바이트 사용
"""
import asyncio
import hashlib
import logging
...
def _generate_point_id(post: CrawledPost) -> int:
    """게시글 point ID 생성. SHA256 전체 8바이트로 충돌 위험 최소화."""
...
def _post_to_embedding_text(post: CrawledPost) -> str:
    """단일 청크 임베딩 텍스트 (하위 호환용). 본문 500자로 제한."""
    return f"직업: {post.직업} | 제목: {post.제목} | {post.본문[:500]}"


def _post_to_chunks(post: CrawledPost) -> list[tuple[int, str, dict]]:
    """게시글을 슬라이딩 윈도우 방식으로 청크 분할.

    Returns:
        [(point_id, embedding_text, payload), ...]
        - point_id: 청크별 고유 ID
        - embedding_text: 임베딩할 텍스트 (제목 + 청크 본문)
        - payload: Qdrant에 저장할 전체 메타데이터
    """
    body = post.본문 or ""
    title = post.제목 or ""
    unique_key = f"{post.직업군}:{post.직업}:{post.post_id or title}"
    ...
    if len(body) <= CHUNK_SIZE:
        ...
        while start < len(body):
            body_chunks.append(body[start : start + CHUNK_SIZE])
    ...
    for idx, chunk_text in enumerate(body_chunks):
        # 청크별 고유 point ID
        chunk_key = f"{unique_key}__chunk{idx}"
        hash_bytes = hashlib.sha256(chunk_key.encode()).digest()
        point_id = int.from_bytes(hash_bytes[:8], byteorder="big") % (2**63)

        # 임베딩 텍스트: 메타데이터 프리픽스 없이 콘텐츠 중심
        ...
        results.append((point_id, embedding_text, payload))
...
class CrawlPipeline:
    ...
    def embed_and_upsert(self, posts: list[CrawledPost]) -> PipelineResult:
        """게시글 목록을 청킹 → 임베딩 캐시 확인 → 신규/변경 청크만 임베딩 → Qdrant 적재."""
        ...
        logger.info("청킹 시작: %d개 게시글", len(posts))
        start = time.time()

        # 1) 모든 청크 생성
        all_chunks: list[tuple[int, str, dict]] = []
        for post in posts:
            all_chunks.extend(_post_to_chunks(post))
        logger.info("총 청크: %d개 (게시글 %d개)", len(all_chunks), len(posts))

        # 2) 임베딩 캐시 확인 — content_hash가 동일한 청크는 스킵
        all_ids = [c[0] for c in all_chunks]
        try:
            existing_hashes = self.store.get_content_hashes(all_ids)
            if not isinstance(existing_hashes, dict):
                existing_hashes = {}
        except Exception as e:
            logger.warning("캐시 조회 실패, 전체 재임베딩 진행: %s", e)
            existing_hashes = {}

        new_chunks = [
            c for c in all_chunks
            if existing_hashes.get(c[0]) != c[2].get("content_hash")
        ]
        skip_count = len(all_chunks) - len(new_chunks)
        result.skipped = skip_count

        if skip_count:
            logger.info("캐시 히트: %d/%d 청크 스킵", skip_count, len(all_chunks))

        if not new_chunks:
            logger.info("변경된 청크 없음 — 임베딩 스킵.")
            result.elapsed_seconds = time.time() - start
            return result

        # 3) 배치 임베딩 & 적재
        total = len(new_chunks)
        logger.info("임베딩 & 적재 시작: %d개 청크", total)
        ...
        for i in range(0, total, self.embedding_batch_size):
            ...
            try:
                ...
                payloads = [c[2] for c in batch]
                self.store.upsert_batch(ids, vectors, payloads, batch_size=self.embedding_batch_size)
        ...
        result.elapsed_seconds = time.time() - start
        logger.info(
            "적재 완료 — 임베딩: %d, 적재: %d, 캐시 스킵: %d, 에러: %d, 소요: %.1f초",
            result.embedded, result.upserted, skip_count, result.errors, result.elapsed_seconds,
        )
        return result
