...
def test_route_query_reads_state_route():
    ...
    assert route_query({"route": ROUTE_BOSS}) == ROUTE_BOSS
...
def test_parse_router_response_with_job_group():
    """JOB/GROUP 추출 — rag 경로에서 직업/직업군 필터 파싱."""
    scores, best, filter_job, filter_group = _parse_router_response(
        "RAG: 5\nBOSS: 1\nCHARACTER_SYNC: 1\n"
        "BEST: rag\nJOB: 팔라딘\nGROUP: 전사"
    )
    assert best == ROUTE_RAG
    assert filter_job == "팔라딘"
    assert filter_group == "전사"


def test_parse_router_response_clamps_scores():
    """6 이상은 5로, 0 이하는 1로."""
    scores, _, _, _ = _parse_router_response("RAG: 9\nBOSS: 0\nBEST: rag")
    assert scores.get("rag") == 5
    assert scores.get("boss") == 1
...
def test_router_agent_node_rag_with_job_sets_filter():
    """RAG 경로에서 JOB 추출 시 filter_job이 state에 포함."""
    router_mod = importlib.import_module("app.agent.router")
    with patch.object(router_mod, "OpenAIClient") as mock_cls:
        mock_ai = MagicMock()
        mock_ai.chat_completion.return_value = (
