...
class TestChunking:
    ...
    def test_short_body_produces_one_chunk(self):
        """짧은 본문은 청크 1개."""
        post = _make_post(본문="짧은 본문")
        chunks = _post_to_chunks(post)
        assert len(chunks) == 1

    def test_long_body_produces_multiple_chunks(self):
        """CHUNK_SIZE 초과 본문은 여러 청크."""
        post = _make_post(본문="가" * (CHUNK_SIZE + 100))
        chunks = _post_to_chunks(post)
        assert len(chunks) > 1

    def test_chunk_payload_has_index(self):
        """각 청크 페이로드에 chunk_index 포함."""
        post = _make_post(본문="가" * (CHUNK_SIZE + 100))
        chunks = _post_to_chunks(post)
        assert chunks[0][2]["chunk_index"] == 0
        assert chunks[1][2]["chunk_index"] == 1

    def test_chunk_payload_has_content_hash(self):
        """각 청크 페이로드에 content_hash 포함."""
        post = _make_post()
        chunks = _post_to_chunks(post)
        assert "content_hash" in chunks[0][2]
        assert len(chunks[0][2]["content_hash"]) == 64  # SHA256 hex

    def test_different_chunks_different_ids(self):
        """동일 게시글의 다른 청크는 다른 point_id."""
        post = _make_post(본문="가" * (CHUNK_SIZE + 100))
        chunks = _post_to_chunks(post)
...
class TestCrawlPipelineEmbedAndUpsert:
    ...
    def test_embed_and_upsert_skips_cached_chunks(self):
        ...
        chunks = _post_to_chunks(post)
        chunk_id = chunks[0][0]
        chunk_hash = chunks[0][2]["content_hash"]

        # 해당 청크가 이미 존재하는 것처럼 반환
        mock_store.get_content_hashes.return_value = {chunk_id: chunk_hash}

        pipeline = CrawlPipeline(openai_client=mock_ai, qdrant_store=mock_store)
        result = pipeline.embed_and_upsert([post])

        # 변경 없음 → 임베딩 API 호출 없음
        mock_ai.create_embeddings_batch.assert_not_called()
        assert result.upserted == 0
