...
def _mock_router_growth(state):
    return {"route": "growth", "route_confidence": 5}
...
def _mock_router_newbie(state):
    return {"route": "newbie", "route_confidence": 5}
...
def test_invoke_growth_route_with_snapshot():
    """라우터 mock으로 growth 분기 → growth 노드 실행, OpenAI mock."""
    reset_graph()
    growth_mod = importlib.import_module("app.agent.nodes.growth_node")
    with patch("app.agent.graph.router_agent_node", side_effect=_mock_router_growth), patch.object(
        growth_mod, "OpenAIClient"
    ) as mock_cls:
        mock_ai = MagicMock()
        mock_ai.chat_completion.return_value = "1. 스타포스 2. 코어 3. 유니온"
    ...
    assert "answer" in result
    assert "1." in result["answer"] or "스타포스" in result["answer"]
...
def test_invoke_newbie_route():
    """라우터 mock으로 newbie 분기 → newbie 노드, 고정 가이드 포함."""
    reset_graph()
    with patch("app.agent.graph.router_agent_node", side_effect=_mock_router_newbie):
        result = invoke(query="뉴비 내실 체우기 가이드")
    assert "answer" in result
    assert "내실" in result["answer"] or "링크" in result["answer"] or "유니온" in result["answer"]
